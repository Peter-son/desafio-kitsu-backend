...
![Spring Boot v2.5.2](https://img.shields.io/badge/Spring%20boot-v2.5.2-green?style=flat&logo=springboot?link=https://spring.io/blog/2022/09/22/spring-boot-2-6-12-available-now&link=https://spring.io/blog/2022/09/22/spring-boot-2-6-12-available-now)
...
![Visual Studio Code v1.72.0](https://img.shields.io/badge/Visual%20Studio%20Code-v1.72.0-blue?logo=Visual%20Studio%20Code&logoColor=blue&link=https://code.visualstudio.com/updates/v1_72)
![Thunder Client v1.20.0](https://img.shields.io/badge/Thunder%20Client-v1.20.0-blueviolet?link=https://marketplace.visualstudio.com/items?itemName=rangav.vscode-thunder-client&link=https://marketplace.visualstudio.com/items?itemName=rangav.vscode-thunder-client)

# Descrição:
Projeto desenvolvido com a linguagem de programação Java orientado a objetos, para o desafio Kitsu, com foco de aplicação backend, da empresa Firedev IT. O sistema realiza consultas na API https://kitsu.docs.apiary.io, permitindo o acompanhamento de determinadas informações sobre animes, mangas e episódios.

# :hammer: Funcionalidades do projeto:

- `Descrição de um anime:` busca e retorno de informações de um anime;
- `Animes em alta:` busca e retorno de animes mais populares;
- `Filtragem de um anime ou manga:` busca e retorno de um anime ou manga com filtros;
- `Descrição de um manga:` busca e retorno de informações de um manga;
- `Mangas em alta:` busca e Retorno dos mangas mais populares;
...
## 📑 Dependências:
- `spring-boot-starter-parent 2.5.2:` fornece configurações iniciais e uma arquitetura com dependências essenciais para inicialização do Spring Boot, juntamente com um gerenciamento de plugins para aplicativos construídos com Maven;
- `spring-boot-starter-web:` para construção de aplicações web, incluindo RESTful, usando Spring MVC;
- `spring-boot-starter-webflux:` para construir aplicativos WebFlux usando o suporte Reactive Web do Spring Framework;
- `spring-boot-devtools:` para gerenciar melhor o tempo de desenvolvimento no Spring Boot, por meio de padrões de propriedade, reinicialização automática, liveReload, túnel de depuração remota, atualização e reinicialização remota;
- `Lombok v1.18.24:` biblioteca Java aplicável para a redução de código, que por meio de anotações, indica ao compilador em meio a um processo, uma maneira de elaborar o código automaticamente;
...
Após executar o projeto e esperar que o mesmo complete o processo de executação, é necessário abrir o link do Swagger http://localhost:8080/swagger-ui/index.html no em um navegador, para uma consulta com uma interface simplificada. 

> :construction: Na interface do Swagger, as opções de consulta '/desafio/filtrando/{escolha}/{atributo}/{valor}', por algum motivo, as informações são apresentadas em ordem alfabética (atributo, escolha e valor), entre tanto a ordem definida no método e que deve ser a ordem de preenchimento é 'escolha, atributo e valor'. Exemplo: 'anime/categories/adventure' ou manga/chapterCount/4 :construction:

> :construction: Para realizar as consultas na opção '/desafio/episodio/{id}', o id que deve ser informado, deverá ser com números acima de 27, por não existirem valores abaixo do valor citado, disponíveis na api Kitsu :construction:


Também é possível realizar as consultas (de uma maneira menos simplificada) na extensão do Thunder Client (na barra esquerda ou apertando o atalho 'Ctrl+shift+R'), adicionar um 'new request' (no botão azul superior esquerdo), aplicar um dos comandos de buscas (que estão listados logo abaixo) e por fim clicar no 'send', para que a consulta seja aplicada. Demosntração a seguir:
...
- `Para obter os animes mais populares, é necessário executar o projeto e colocar o seguinte link no Thunder Client:` http://localhost:8080/desafio/animesAlta
![animesAlta](https://user-images.githubusercontent.com/75283449/198038239-9aade65d-79e3-4316-819a-c6aab30dbc00.png)
...
- `Para obter os mangas mais populares, é necessário executar o projeto e colocar o seguinte link no Thunder Client:`  http://localhost:8080/desafio/mangasAlta       
...
Exemplo de filtros por texto:

![filtroTexto](https://user-images.githubusercontent.com/75283449/198047017-963b4204-84b4-473e-9830-6d6b4822f3f6.png)

Exemplos de filtros para anime:

![filtrosAnime](https://user-images.githubusercontent.com/75283449/198041686-e0854c9f-341b-4ca2-87fe-989292a29fb5.png)

Exemplo de uso: http://localhost:8080/desafio/filtrando/anime/categories/adventure
![filtraAnime](https://user-images.githubusercontent.com/75283449/198044826-0b3ce64c-bc54-46fd-ac66-ae69592b9e74.png)

Exemplos de filtros para manga: 

![filtroManga](https://user-images.githubusercontent.com/75283449/198046265-68c69c05-5caa-4594-9fec-24770fe5d0f7.png)
 
Exemplo de uso: http://localhost:8080/desafio/filtrando/manga/chapterCount/4
![filtraManga](https://user-images.githubusercontent.com/75283449/198046398-21187a81-d8af-4716-a0e3-e0e71b129953.png)

- `Para obter informações de um episódio de um anime específico, é necessário executar o projeto e colocar o seguinte link no Thunder Client:` http://localhost:8080/desafio/episodio/id (onde o {id} deverá ser subistituído por um número inteiro, acima de 27). A estrutura utilizada para retorno das informações (Classes e variáveis) é a mesma abordada para o retorno de animes, para fins de simplificação, dessa forma são recuperadas apenas parte das informações sobre 'episodes'.
![episodioId](https://user-images.githubusercontent.com/75283449/198101666-b2b0943b-ba66-4764-aa93-c31055c65a4b.png)
